function val = integrate_sparse(c4n, n4p, integrand, degree, OPTsize4parts)
...
%                         or the reference triangle
    ...
    for curGpt = 1:nrGpts
        ...
        if (curGpt == 1)
            K_total = size(integrandVal, 2);
            val = sparse(nrParts, K_total); % Keep it sparse!
        end

        % Apply the weight using a sparse diagonal matrix
        curGwt4p = weightFactor * Gwts4ref(curGpt); 
        W = spdiags(curGwt4p, 0, nrParts, nrParts);
        
        val = val + W * integrandVal;
    end
end
